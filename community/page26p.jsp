...
<c:if test="${not(empty mssu2p) and (mssu2p.dataSize gt 0)}">
	<div class="alert alert-info ">
		<c:forEach var="current" varStatus="status" items="${mssu2p.data}">
		...
		<p>
			<c:if test="${webratio:isTargetAccessible('ln145p', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln145p', pageContext)}">
	 		<a title="Ok" id="ln145p" onclick="return ajaxRequest('<webratio:Link link="ln145p" position="index"/>', $H({pressedLink: 'ln145p', sourcePage: 'page26p'}))" class=" " href="<webratio:Link link="ln145p" position="index"/>">							
			Ok</a>
	</c:if>
			</c:if>
		</p>
	...
	<script type="application/json" class="wr-linkInfos">
		<webratio:LinkInfos page="page26p"/>
	</script>
	<script type="application/json" class="wr-linkData">
		<webratio:LinkData page="page26p"/>
	</script>
