...
<div class="container container-fixed-top-padding">
	<ol class="breadcrumb" aria-label="breadcrumbs">
		 <li><c:if test="${webratio:isTargetAccessible('page12q.link', pageContext)}">	
 <a class="small" href="<webratio:Link link="page12q.link"/>">
      <bean:message key="Rewards"/>
</a>
			</c:if>
 </li>
		<li class="active"><bean:message key="Error"/></li>
	</ol>
...
<c:if test="${not(empty mssu3q) and (mssu3q.dataSize gt 0)}">
	<div class="alert alert-info ">
		<c:forEach var="current" varStatus="status" items="${mssu3q.data}">
      ...
      <table align="left">
        <tr>
           <td>
             <table>
                 <tr>
			<c:if test="${webratio:isTargetAccessible('ln164q', pageContext)}">
                 <td nowrap="nowrap">
                    <c:if test="${webratio:isTargetAccessible('ln164q', pageContext)}">
	 		<a title="<bean:message key="Go Back"/>" id="ln164q" class="  link" href="<webratio:Link link="ln164q" position="index"/>" >
