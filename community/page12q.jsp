...
<webratio:Page page="page12q"/>
<html lang="${LanguageISOCtxParam}">
	<head>
		<meta http-equiv="content-type" content="text/html; charset=utf-8">
	    <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<base href="<%=wrBaseURI%>"/> 
		<title><bean:message key="Rewards"/></title>
...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page12q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page12qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page12q"/>
...
<div class="container container-fixed-top-padding">
	<div class="page-header clearfix">
			<h1><bean:message key="Rewards"/></h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<c:if test="${not(empty pwu5q) and (pwu5q.dataSize gt 0)}">
	...
	<div class="panel-body">
		<div class="plain ">
			<div class="plain PowerIndexUnit">
				<c:forEach var="current" varStatus="status" items="${pwu5q.data}">
					<c:set var="index" value="${status.index}"/>
					<c:if test="${index % 3 == 0}">
						<div>
					</c:if>	
						<div class="rewardTable">
						<table class="full-width row<c:if test="${pwu5q.currentIndex eq index}">Current</c:if>">	
									<tr>
									   <td class=" value<c:if test="${pwu5q.currentIndex eq index}">Current</c:if> blob">
											<c:if test="${not (empty current.image)}">
		<img   src="<webratio:BLOB value="${current.image}"/>" alt="<c:out value="${current.image.name}"/>" title="<c:out value="${current.image.name}"/>"/> 
  </c:if>
									   </td>
									</tr>
									<tr>
									   <td class=" value<c:if test="${pwu5q.currentIndex eq index}">Current</c:if> string">
												<span class="rewardTableTitle"><c:out value="${current.title}"/></span>		
									   </td>
									</tr>
									<tr>
									   <td class=" value<c:if test="${pwu5q.currentIndex eq index}">Current</c:if> decimal">
												<span class="rewardTablePoints"><fmt:formatNumber value="${current.neededPoints}" pattern="${decimalPattern}"/> <bean:message key="Points"/></span>
									   </td>
									</tr>
			<c:if test="${webratio:isTargetAccessible('ln155q', pageContext)}">
							        <tr>
										<td class=" value<c:if test="${pwu5q.currentIndex eq index}">Current</c:if>">
											<c:choose>
								   				<c:when test="${pwu5q.currentIndex eq index}">
													 <c:if test="${webratio:isTargetAccessible('ln155q', pageContext)}">
	 		<a title="<bean:message key="View Reward"/>" id="ln155q" class="btn  btn-default  linkCurrent" href="<webratio:Link link="ln155q" position="index"/>" >
			<bean:message key="View Reward"/></a>
	</c:if>
								   				</c:when>
								   				<c:otherwise>
								   					 <c:if test="${webratio:isTargetAccessible('ln155q', pageContext)}">
	 		<a title="<bean:message key="View Reward"/>" id="ln155q" class="btn  btn-default  link" href="<webratio:Link link="ln155q" position="index"/>" >
			<bean:message key="View Reward"/></a>
	</c:if>
								   				</c:otherwise>
								   			</c:choose>
...
<c:choose>
<c:when test="${not(empty dau7q) and (dau7q.dataSize gt 0)}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
			<bean:message key="Rewards Area"/>
		</h2>
	</div>
	<div class="panel-body">
		<div class="">
				<div class="value"><c:out value="${dau7q.data.message}" escapeXml="false"/></div>
		</div>
	</div>
</div>     
</c:when>
