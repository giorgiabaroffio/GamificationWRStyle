...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page24p.do${wrAjax ? '#!ajax=true' : ''}" styleId="page24pFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page24p"/>
				<input type="hidden" name="ln75p" value="<webratio:Link link="ln75p"/>">
				<input type="hidden" name="ln76p" value="<webratio:Link link="ln76p"/>">
...
<div class="EntryUnit ">
	<html:hidden property="enu14pKey" styleId="enu14pKey"></html:hidden>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu14p"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu14p"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="sfld20p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld20p" class="control-label col-md-2">language</label>
					<div class="controls col-md-10">
					<html:select styleId="sfld20p" styleClass="wr-submitButtons:ln76p,ln75p form-control " property="sfld20p" disabled="false">
				  		<html:option value=""><bean:message key="selectionfield.noselection"/></html:option>
				    <html:options property="sfld20pList" labelProperty="sfld20pLabelList"/>
				</html:select>
					<c:set var="btFieldError"><html:errors property="sfld20p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld35p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld35p" class="control-label col-md-2">Title</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld35p" styleClass="wr-submitButtons:ln76p,ln75p  form-control " property="fld35p" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld35p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld36p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld36p" class="control-label col-md-2">Description</label>
					<div class="controls col-md-10">
					<html:textarea styleId="fld36p" styleClass="form-control " rows="8" property="fld36p" readonly="false"/>		
			<script type="text/javascript">
				if (CKEDITOR.instances['fld36p']) {
					CKEDITOR.instances['fld36p'].destroy(true);
				}
				CKEDITOR.replace('fld36p', {
					width: '100%',
					height   : 200,
					...
					<c:set var="btFieldError"><html:errors property="fld36p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld47p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld47p" class="control-label col-md-2">Image</label>
					<div class="controls col-md-10">
					<html:hidden property="fld47p_preload" styleId="fld47p_preload"/>
			<html:file styleId="fld47p" styleClass="wr-submitButtons:ln76p,ln75p " property="fld47p" accept="" disabled="false" />
		<c:if test="${not(empty page24pFormBean.map.fld47p_preload)}">
			<label class="input-group-addon blob-clear" for="fld47p_clear"><html:checkbox styleClass="wr-submitButtons:ln76p,ln75p " styleId="fld47p_clear" property="fld47p_clear" disabled="false"/> Clear </label><div class="input-group-btn"><a target="blank" class="btn btn-default" href="<webratio:BLOB value="${page24pFormBean.map.fld47p_preload}"/>">Preview</a></div>
		</c:if>
					<c:set var="btFieldError"><html:errors property="fld47p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld37p_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld37p_locale" class="control-label col-md-2">Needed Points</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld37p_locale" styleClass="wr-submitButtons:ln76p,ln75p  form-control " property="fld37p_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld37p_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld38p_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld38p_locale" class="control-label col-md-2">Available</label>
					<div class="controls col-md-10">
					<label class="radio-inline" for="bool_fld38p_0">
  					<html:radio styleClass="wr-submitButtons:ln76p,ln75p " styleId="bool_fld38p_0" property="fld38p_locale" value="true" disabled="false"/>
					<bean:message key="boolean.yes"/>
				</label>
				<label class="radio-inline" for="bool_fld38p_1">
  					<html:radio styleClass="wr-submitButtons:ln76p,ln75p " styleId="bool_fld38p_1" property="fld38p_locale" value="false" disabled="false"/>
					<bean:message key="boolean.no"/>
				</label>
		<c:if test="false">
			<html:hidden property="fld38p_locale"/>
		</c:if>
					<c:set var="btFieldError"><html:errors property="fld38p_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln76p', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln76p', pageContext)}">
			<button title="Save" onclick="$('#page24pFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln76p" name="button:ln76p" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln75p', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln75p', pageContext)}">
			<button title="Back" onclick="$('#page24pFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln75p" name="button:ln75p" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
