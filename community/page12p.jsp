...
<webratio:Page page="page12p"/>
<html lang="${LanguageISOCtxParam}">
	<head>
		<meta http-equiv="content-type" content="text/html; charset=utf-8">
	    <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<base href="<%=wrBaseURI%>"/> 
		<title><bean:message key="HQ.UserDashboard.Private"/></title>
...
<div class="container container-fixed-top-padding">
	<div class="page-header clearfix">
			<h1><bean:message key="HQ.UserDashboard.Private"/></h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div id="profile"  class="box compact">
	<div class="box-title">
		<c:set var="username" value="${dau5p.data.firstName} ${dau5p.data.lastName}"/>
    	<h2 style="color:black !important">${username}&nbsp;</h2>
    	<div class="photo">
			<c:choose>
				<c:when test="${not empty dau5p.data.bigPhoto}">
					<img src="<webratio:BLOB value="${dau5p.data.bigPhoto}" />" alt="${username}"/> 
				</c:when>
				<c:otherwise>
		...
		<div class="information">
			<c:if test="${not empty(dau5p.data.companyName)}">
						<bean:message key="HQ.YourProfile.Work"/> <bean:message key="HQ.YourProfile.At"/> <c:out value="${dau5p.data.companyName}"/>
			</c:if>
			<br/>
			<c:if test="${not empty(dau5p.data.city)}">
				<c:out value="${dau5p.data.city}"/>
			</c:if>
			<c:if test="${not empty(dau5p.data.country)}">
				<c:if test="${not empty(dau5p.data.city)}">, </c:if>
				<c:out value="${dau5p.data.country}"/>
			</c:if>
		</div>
	</div>
</div>
			...
			<div class="grid_4 suffix_4 omega agrd_8">
	<!--PARTICIPATION BOX-->
	<div  id="participation-box" class="box one-third">
		<img src="BootstrapStyle/img/7Days-Leaderboard.png"/>
	</div>
	<div class="private-message"><h2>
<c:if test="${not(empty dau4p) and (dau4p.dataSize gt 0)}">
		<div class="">
				<div class="value"><c:out value="${dau4p.data.message}" escapeXml="false"/></div>
		</div>
</c:if>
   </h2></div>
	<!--END PARTICIPATION--> 
	<!--CREDIT BOX-->
	<div class="box light compact credits-box" style="margin-top:-20px;">
		<img src="BootstrapStyle/img/Points.png"/>
	</div>
	<!--END CREDITS-->
			</div>
				<div class="clear"></div>
