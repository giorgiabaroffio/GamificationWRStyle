...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page25q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page25qFormBean" enctype="multipart/form-data">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">Manage Thematic Area</li>
	</ol>
	<div class="page-header clearfix">
			<h1>Manage Thematic Area</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<c:if test="${not(empty pwu3q) and (pwu3q.dataSize gt 0)}">
<div class="left-bar">
...
<th scope="col" class=" smallcol"></th> 
				...
				<c:forEach var="current" varStatus="status" items="${pwu3q.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr <c:if test="${pwu3q.currentIndex eq index}">class="info"</c:if>>	
								<td class=" string">
			...
			<c:if test="${webratio:isTargetAccessible('ln86q', pageContext)}">
									<td class=" smallcol">
										 <c:if test="${webratio:isTargetAccessible('ln86q', pageContext)}">
	 		<a title="Modify" id="ln86q" class=" " href="<webratio:Link link="ln86q" position="index"/>" >
			Modify</a>
	</c:if>
									</td>
			</c:if>
	...
	<% pageContext.setAttribute("_wr_message", org.apache.commons.lang.StringEscapeUtils.escapeJavaScript((String) pageContext.getAttribute("_wr_message")));%>
		<c:if test="${webratio:isTargetAccessible('ln87q', pageContext)}">
	 		<a title="Delete" id="ln87q" onclick=" var c = confirm('<c:out value="${_wr_message}"/>'); if (c) { return ajaxRequest('<webratio:Link link="ln87q" position="index"/>', $H({pressedLink: 'ln87q', sourcePage: 'page25q'})) }  return c; " class=" " href="<webratio:Link link="ln87q" position="index"/>">							
			Delete</a>
	</c:if>
									</td>
			</c:if>
