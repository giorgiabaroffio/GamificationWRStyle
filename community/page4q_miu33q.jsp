...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page4q_miu33q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page4q_miu33qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page4q_miu33q"/>
				<input type="hidden" name="ln146q" value="<webratio:Link link="ln146q"/>">
				<input type="hidden" name="ln149q" value="<webratio:Link link="ln149q"/>">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">Modify Actions</li>
	</ol>
...
<div class="EntryUnit ">
	<html:hidden property="enu3qKey" styleId="enu3qKey"></html:hidden>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu3q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu3q"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld5q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld5q" class="control-label col-md-2">Name</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld5q" styleClass="wr-submitButtons:ln146q,ln149q  form-control " property="fld5q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld5q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld6q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld6q" class="control-label col-md-2">Description</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld6q" styleClass="wr-submitButtons:ln146q,ln149q  form-control " property="fld6q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld6q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld4q_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld4q_locale" class="control-label col-md-2">Score</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld4q_locale" styleClass="wr-submitButtons:ln146q,ln149q  form-control " property="fld4q_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld4q_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="sfld1q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld1q" class="control-label col-md-2">Participation</label>
					<div class="controls col-md-10">
					<label class="radio-inline" for="bool_sfld1q_0">
					<html:radio property="sfld1q" value="true" styleId="bool_sfld1q_0" styleClass="wr-submitButtons:ln146q,ln149q" disabled="false">
		      		  <bean:message key="boolean.yes"/>
		      		</html:radio>
				</label>
				<label class="radio-inline" for="bool_sfld1q_1">
	  				<html:radio property="sfld1q" value="false" styleId="bool_sfld1q_1" styleClass="wr-submitButtons:ln146q,ln149q" disabled="false">
		      		  <bean:message key="boolean.no"/>
		      		</html:radio>
				</label>
					<c:set var="btFieldError"><html:errors property="sfld1q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="sfld2q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld2q" class="control-label col-md-2">Reputation</label>
					<div class="controls col-md-10">
					<label class="radio-inline" for="bool_sfld2q_0">
					<html:radio property="sfld2q" value="true" styleId="bool_sfld2q_0" styleClass="wr-submitButtons:ln146q,ln149q" disabled="false">
		      		  <bean:message key="boolean.yes"/>
		      		</html:radio>
				</label>
				<label class="radio-inline" for="bool_sfld2q_1">
	  				<html:radio property="sfld2q" value="false" styleId="bool_sfld2q_1" styleClass="wr-submitButtons:ln146q,ln149q" disabled="false">
		      		  <bean:message key="boolean.no"/>
		      		</html:radio>
				</label>
					<c:set var="btFieldError"><html:errors property="sfld2q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="sfld3q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld3q" class="control-label col-md-2">Repeatable</label>
					<div class="controls col-md-10">
					<label class="radio-inline" for="bool_sfld3q_0">
					<html:radio property="sfld3q" value="true" styleId="bool_sfld3q_0" styleClass="wr-submitButtons:ln146q,ln149q" disabled="false">
		      		  <bean:message key="boolean.yes"/>
		      		</html:radio>
				</label>
				<label class="radio-inline" for="bool_sfld3q_1">
	  				<html:radio property="sfld3q" value="false" styleId="bool_sfld3q_1" styleClass="wr-submitButtons:ln146q,ln149q" disabled="false">
		      		  <bean:message key="boolean.no"/>
		      		</html:radio>
				</label>
					<c:set var="btFieldError"><html:errors property="sfld3q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="sfld4q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld4q" class="control-label col-md-2">Check Time Elapsed</label>
					<div class="controls col-md-10">
					<label class="radio-inline" for="bool_sfld4q_0">
					<html:radio property="sfld4q" value="true" styleId="bool_sfld4q_0" styleClass="wr-submitButtons:ln146q,ln149q" disabled="false">
		      		  <bean:message key="boolean.yes"/>
		      		</html:radio>
				</label>
				<label class="radio-inline" for="bool_sfld4q_1">
	  				<html:radio property="sfld4q" value="false" styleId="bool_sfld4q_1" styleClass="wr-submitButtons:ln146q,ln149q" disabled="false">
		      		  <bean:message key="boolean.no"/>
		      		</html:radio>
				</label>
					<c:set var="btFieldError"><html:errors property="sfld4q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld10q_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld10q_locale" class="control-label col-md-2">Time Elapsed (minutes)</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld10q_locale" styleClass="wr-submitButtons:ln146q,ln149q  form-control " property="fld10q_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld10q_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld11q_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld11q_locale" class="control-label col-md-2">Active</label>
					<div class="controls col-md-10">
					<label class="radio-inline" for="bool_fld11q_0">
  					<html:radio styleClass="wr-submitButtons:ln146q,ln149q " styleId="bool_fld11q_0" property="fld11q_locale" value="true" disabled="false"/>
					<bean:message key="boolean.yes"/>
				</label>
				<label class="radio-inline" for="bool_fld11q_1">
  					<html:radio styleClass="wr-submitButtons:ln146q,ln149q " styleId="bool_fld11q_1" property="fld11q_locale" value="false" disabled="false"/>
					<bean:message key="boolean.no"/>
				</label>
		<c:if test="false">
			<html:hidden property="fld11q_locale"/>
		</c:if>
					<c:set var="btFieldError"><html:errors property="fld11q_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln146q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln146q', pageContext)}">
			<button title="Update" onclick="$('#page4q_miu33qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln146q" name="button:ln146q" type="submit" value="Update">
			Update</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln149q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln149q', pageContext)}">
			<button title="Back" onclick="$('#page4q_miu33qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln149q" name="button:ln149q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
