...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page18x.do${wrAjax ? '#!ajax=true' : ''}" styleId="page18xFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page18x"/>
				<input type="hidden" name="ln66x" value="<webratio:Link link="ln66x"/>">
				<input type="hidden" name="ln136x" value="<webratio:Link link="ln136x"/>">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">Modify Badge Area</li>
	</ol>
	<div class="page-header clearfix">
			<h1>Modify Badge Area</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div class="EntryUnit ">
		<html:hidden property="fld55x" styleId="fld55x"/>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu15x"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu15x"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld53x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld53x" class="control-label col-md-2">Sort Number</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld53x" styleClass="wr-submitButtons:ln136x,ln66x  form-control " property="fld53x" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld53x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld52x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld52x" class="control-label col-md-2">Name</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld52x" styleClass="wr-submitButtons:ln136x,ln66x  form-control " property="fld52x" readonly="true"/>
					<c:set var="btFieldError"><html:errors property="fld52x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln136x', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln136x', pageContext)}">
			<button title="Update" onclick="$('#page18xFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln136x" name="button:ln136x" type="submit" value="Update">
			Update</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln66x', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln66x', pageContext)}">
			<button title="Back" onclick="$('#page18xFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln66x" name="button:ln66x" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
