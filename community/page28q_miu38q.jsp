...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page28q_miu38q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page28q_miu38qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page28q_miu38q"/>
				<input type="hidden" name="ln134q" value="<webratio:Link link="ln134q"/>">
				<input type="hidden" name="ln135q" value="<webratio:Link link="ln135q"/>">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">Edit Thematic Area</li>
	</ol>
...
<div class="EntryUnit ">
		<html:hidden property="fld40q" styleId="fld40q"/>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu16q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu16q"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld39q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld39q" class="control-label col-md-2">Area Name</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld39q" styleClass="wr-submitButtons:ln134q,ln135q  form-control " property="fld39q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld39q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="msfld7q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="msfld7q" class="control-label col-md-2">Applications Associated</label>
					<div class="controls col-md-10">
					<c:forEach var="current" varStatus="status" items="${page28q_miu38qFormBean.map.msfld7qList}">
				 <label class="checkbox-inline" for="msf_msfld7q_${status.index}">
				  <html:multibox styleClass="wr-submitButtons:ln134q,ln135q " property="msfld7q" value="${current}" disabled="false" styleId="msf_msfld7q_${status.index}"/>
					   <c:out value="${page28q_miu38qFormBean.map.msfld7qLabelList[status.index]}"/>
				 </label>
			</c:forEach>
					<c:set var="btFieldError"><html:errors property="msfld7q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln134q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln134q', pageContext)}">
			<button title="Save" onclick="$('#page28q_miu38qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln134q" name="button:ln134q" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln135q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln135q', pageContext)}">
			<button title="Back" onclick="$('#page28q_miu38qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln135q" name="button:ln135q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
