...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page38p.do${wrAjax ? '#!ajax=true' : ''}" styleId="page38pFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page38p"/>
				<input type="hidden" name="ln47p" value="<webratio:Link link="ln47p"/>">
				<input type="hidden" name="ln115p" value="<webratio:Link link="ln115p"/>">
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu10p"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu10p"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="sfld9p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld9p" class="control-label col-md-2">language</label>
					<div class="controls col-md-10">
					<html:select styleId="sfld9p" styleClass="wr-submitButtons:ln115p,ln47p form-control " property="sfld9p" disabled="false">
				  		<html:option value=""><bean:message key="selectionfield.noselection"/></html:option>
				    <html:options property="sfld9pList" labelProperty="sfld9pLabelList"/>
				</html:select>
					<c:set var="btFieldError"><html:errors property="sfld9p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld8p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld8p" class="control-label col-md-2">Alias</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld8p" styleClass="wr-submitButtons:ln115p,ln47p  form-control " property="fld8p" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld8p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld1p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld1p" class="control-label col-md-2">Text</label>
					<div class="controls col-md-10">
					<html:textarea styleId="fld1p" styleClass="form-control " rows="8" property="fld1p" readonly="false"/>		
			<script type="text/javascript">
				if (CKEDITOR.instances['fld1p']) {
					CKEDITOR.instances['fld1p'].destroy(true);
				}
				CKEDITOR.replace('fld1p', {
					width: '100%',
					height   : 200,
					...
					<c:set var="btFieldError"><html:errors property="fld1p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln115p', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln115p', pageContext)}">
			<button title="Save" onclick="$('#page38pFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln115p" name="button:ln115p" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln47p', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln47p', pageContext)}">
			<button title="Back" onclick="$('#page38pFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln47p" name="button:ln47p" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
