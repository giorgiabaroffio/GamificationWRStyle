...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page4x.do${wrAjax ? '#!ajax=true' : ''}" styleId="page4xFormBean" enctype="multipart/form-data">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">Manage Badge Area</li>
	</ol>
	<div class="page-header clearfix">
			<h1>Manage Badge Area</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<tr>
<td  valign="top" align="left" >
<c:if test="${not(empty pwu6x) and (pwu6x.dataSize gt 0)}">
...
<th scope="col" class=" smallcol"></th> 
				...
				<c:forEach var="current" varStatus="status" items="${pwu6x.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr <c:if test="${pwu6x.currentIndex eq index}">class="info"</c:if>>	
								<td class=" integer">
			...
			<c:if test="${webratio:isTargetAccessible('ln62x', pageContext)}">
									<td class=" smallcol">
										 <c:if test="${webratio:isTargetAccessible('ln62x', pageContext)}">
	 		<a title="Modify" id="ln62x" class=" " href="<webratio:Link link="ln62x" position="index"/>" >
			Modify</a>
	</c:if>
									</td>
			</c:if>
