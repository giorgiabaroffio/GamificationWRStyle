...
<webratio:Page page="page27q"/>
<html lang="${LanguageISOCtxParam}">
	<head>
		<meta http-equiv="content-type" content="text/html; charset=utf-8">
	    <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<base href="<%=wrBaseURI%>"/> 
		<title>Modify Gamified Application</title>
...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page27q_miu29q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page27q_miu29qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page27q_miu29q"/>
				<input type="hidden" name="ln118q" value="<webratio:Link link="ln118q"/>">
				<input type="hidden" name="ln127q" value="<webratio:Link link="ln127q"/>">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">Modify Gamified Application</li>
	</ol>
	<div class="page-header clearfix">
			<h1>Modify Gamified Application</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div class="EntryUnit ">
		<html:hidden property="fld37q" styleId="fld37q"/>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu15q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu15q"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld38q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld38q" class="control-label col-md-2">Name</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld38q" styleClass="wr-submitButtons:ln118q,ln127q  form-control " property="fld38q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld38q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="msfld6q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="msfld6q" class="control-label col-md-2">Thematic Area Associated</label>
					<div class="controls col-md-10">
					<c:forEach var="current" varStatus="status" items="${page27q_miu29qFormBean.map.msfld6qList}">
				 <label class="checkbox-inline" for="msf_msfld6q_${status.index}">
				  <html:multibox styleClass="wr-submitButtons:ln118q,ln127q " property="msfld6q" value="${current}" disabled="false" styleId="msf_msfld6q_${status.index}"/>
					   <c:out value="${page27q_miu29qFormBean.map.msfld6qLabelList[status.index]}"/>
				 </label>
			</c:forEach>
					<c:set var="btFieldError"><html:errors property="msfld6q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln118q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln118q', pageContext)}">
			<button title="Update" onclick="$('#page27q_miu29qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln118q" name="button:ln118q" type="submit" value="Update">
			Update</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln127q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln127q', pageContext)}">
			<button title="Back" onclick="$('#page27q_miu29qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln127q" name="button:ln127q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
