...
<webratio:Page page="page11q"/>
<html lang="${LanguageISOCtxParam}">
	<head>
		<meta http-equiv="content-type" content="text/html; charset=utf-8">
	    <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<base href="<%=wrBaseURI%>"/> 
		<title>Create New Administrator User</title>
...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page11q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page11qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page11q"/>
				<input type="hidden" name="ln115q" value="<webratio:Link link="ln115q"/>">
				<input type="hidden" name="ln116q" value="<webratio:Link link="ln116q"/>">
 ...
 <a class="" href="<webratio:Link link="page22p.link"/>">
   Manage Rewards
			...
			<c:if test="${webratio:isTargetAccessible('page7q.link', pageContext)}">
								<li class="active"><a href="<webratio:Link escapeXml="true" link="page7q.link"/>">Administrator Users Area</a></li>
			</c:if>
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page7q.link"/>">
   Administrator Users Area
</a>
			</c:if>
 </li>
		<li class="active">Create New Administrator User</li>
	</ol>
	<div class="page-header clearfix">
			<h1>Create New Administrator User</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu9q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu9q"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld20q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld20q" class="control-label col-md-2">Username</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld20q" styleClass="wr-submitButtons:ln116q,ln115q  form-control " property="fld20q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld20q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld19q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld19q" class="control-label col-md-2">Password</label>
					<div class="controls col-md-10">
					<input type="password" autocomplete="off" id="fld19q" class="wr-submitButtons:ln116q,ln115q form-control " name="fld19q"/>
					<c:set var="btFieldError"><html:errors property="fld19q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld18q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld18q" class="control-label col-md-2">Email</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld18q" styleClass="wr-submitButtons:ln116q,ln115q  form-control " property="fld18q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld18q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln116q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln116q', pageContext)}">
			<button title="Save" onclick="$('#page11qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln116q" name="button:ln116q" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln115q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln115q', pageContext)}">
			<button title="Back" onclick="$('#page11qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln115q" name="button:ln115q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
