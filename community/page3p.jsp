...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page3p.do${wrAjax ? '#!ajax=true' : ''}" styleId="page3pFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page3p"/>
				<input type="hidden" name="ln133p" value="<webratio:Link link="ln133p"/>">
				<input type="hidden" name="ln134p" value="<webratio:Link link="ln134p"/>">
...
<div class="EntryUnit ">
		<html:hidden property="fld44p" styleId="fld44p"/>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu15p"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu15p"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="sfld10p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld10p" class="control-label col-md-2">language</label>
					<div class="controls col-md-10">
					<html:select styleId="sfld10p" styleClass="wr-submitButtons:ln134p,ln133p form-control " property="sfld10p" disabled="false">
				  		<html:option value=""><bean:message key="selectionfield.noselection"/></html:option>
				    <html:options property="sfld10pList" labelProperty="sfld10pLabelList"/>
				</html:select>
					<c:set var="btFieldError"><html:errors property="sfld10p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld16p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld16p" class="control-label col-md-2">Alias</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld16p" styleClass="wr-submitButtons:ln134p,ln133p  form-control " property="fld16p" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld16p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld12p"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld12p" class="control-label col-md-2">Text</label>
					<div class="controls col-md-10">
					<html:textarea styleId="fld12p" styleClass="form-control " rows="8" property="fld12p" readonly="false"/>		
			<script type="text/javascript">
				if (CKEDITOR.instances['fld12p']) {
					CKEDITOR.instances['fld12p'].destroy(true);
				}
				CKEDITOR.replace('fld12p', {
					width: '100%',
					height   : 200,
					...
					<c:set var="btFieldError"><html:errors property="fld12p"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln134p', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln134p', pageContext)}">
			<button title="Save" onclick="$('#page3pFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln134p" name="button:ln134p" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln133p', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln133p', pageContext)}">
			<button title="Back" onclick="$('#page3pFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln133p" name="button:ln133p" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
