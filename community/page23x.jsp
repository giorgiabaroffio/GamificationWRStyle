...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page23x.do${wrAjax ? '#!ajax=true' : ''}" styleId="page23xFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page23x"/>
				<input type="hidden" name="ln223x" value="<webratio:Link link="ln223x"/>">
				<input type="hidden" name="ln188x" value="<webratio:Link link="ln188x"/>">
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu17x"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu17x"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld66x_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld66x_locale" class="control-label col-md-2">Level</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld66x_locale" styleClass="wr-submitButtons:ln188x,ln223x  form-control " property="fld66x_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld66x_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld68x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld68x" class="control-label col-md-2">Title</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld68x" styleClass="wr-submitButtons:ln188x,ln223x  form-control " property="fld68x" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld68x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld69x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld69x" class="control-label col-md-2">Description</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld69x" styleClass="wr-submitButtons:ln188x,ln223x  form-control " property="fld69x" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld69x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld67x_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld67x_locale" class="control-label col-md-2">Needed Score</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld67x_locale" styleClass="wr-submitButtons:ln188x,ln223x  form-control " property="fld67x_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld67x_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="sfld7x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld7x" class="control-label col-md-2">Area</label>
					<div class="controls col-md-10">
					<html:select styleId="sfld7x" styleClass="wr-submitButtons:ln188x,ln223x form-control " property="sfld7x" disabled="false">
				    <html:options property="sfld7xList" labelProperty="sfld7xLabelList"/>
				</html:select>
					<c:set var="btFieldError"><html:errors property="sfld7x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld63x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld63x" class="control-label col-md-2">Image (png)</label>
					<div class="controls col-md-10">
					<html:hidden property="fld63x_preload" styleId="fld63x_preload"/>
			<html:file styleId="fld63x" styleClass="wr-submitButtons:ln188x,ln223x " property="fld63x" accept="image/*" disabled="false" />
		<c:if test="${not(empty page23xFormBean.map.fld63x_preload)}">
			<label class="input-group-addon blob-clear" for="fld63x_clear"><html:checkbox styleClass="wr-submitButtons:ln188x,ln223x " styleId="fld63x_clear" property="fld63x_clear" disabled="false"/> Clear </label><div class="input-group-btn"><a target="blank" class="btn btn-default" href="<webratio:BLOB value="${page23xFormBean.map.fld63x_preload}"/>">Preview</a></div>
		</c:if>
					<c:set var="btFieldError"><html:errors property="fld63x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld23q_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld23q_locale" class="control-label col-md-2">Active</label>
					<div class="controls col-md-10">
					<label class="radio-inline" for="bool_fld23q_0">
  					<html:radio styleClass="wr-submitButtons:ln188x,ln223x " styleId="bool_fld23q_0" property="fld23q_locale" value="true" disabled="false"/>
					<bean:message key="boolean.yes"/>
				</label>
				<label class="radio-inline" for="bool_fld23q_1">
  					<html:radio styleClass="wr-submitButtons:ln188x,ln223x " styleId="bool_fld23q_1" property="fld23q_locale" value="false" disabled="false"/>
					<bean:message key="boolean.no"/>
				</label>
		<c:if test="false">
			<html:hidden property="fld23q_locale"/>
		</c:if>
					<c:set var="btFieldError"><html:errors property="fld23q_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln188x', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln188x', pageContext)}">
			<button title="Save" onclick="$('#page23xFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln188x" name="button:ln188x" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln223x', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln223x', pageContext)}">
			<button title="Back" onclick="$('#page23xFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln223x" name="button:ln223x" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
