...
<webratio:Page page="page7q"/>
<html lang="${LanguageISOCtxParam}">
	<head>
		<meta http-equiv="content-type" content="text/html; charset=utf-8">
	    <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<base href="<%=wrBaseURI%>"/> 
		<title>Manage Administrator Users</title>
...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page7q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page7qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page7q"/>
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page7q.link"/>">
   Administrator Users Area
		...
		<li class="active">Manage Administrator Users</li>
	</ol>
	<div class="page-header clearfix">
			<h1>Manage Administrator Users</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<c:if test="${not(empty pwu4q) and (pwu4q.dataSize gt 0)}">
<div class="panel panel-default ">
	...
	<div class="panel-body">
		<div class="plain ">
			...
			<c:if test="${webratio:isTargetAccessible('ln109q', pageContext)}">
<th scope="col" class=" smallcol"></th> 
			</c:if>
						</tr>
					</thead>
				<tbody>
				<c:forEach var="current" varStatus="status" items="${pwu4q.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr <c:if test="${pwu4q.currentIndex eq index}">class="info"</c:if>>	
								<td class=" string">
			...
			<c:if test="${webratio:isTargetAccessible('ln109q', pageContext)}">
									<td class=" smallcol">
										 <c:if test="${webratio:isTargetAccessible('ln109q', pageContext)}">
	 		<a title="Modify" id="ln109q" class=" " href="<webratio:Link link="ln109q" position="index"/>" >
			Modify</a>
	</c:if>
									</td>
			</c:if>
