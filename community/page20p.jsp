...
<webratio:Page page="page20p"/>
<html lang="${LanguageISOCtxParam}">
	<head>
		<meta http-equiv="content-type" content="text/html; charset=utf-8">
	    <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<base href="<%=wrBaseURI%>"/> 
		<title><bean:message key="HQ.MyDashboard "/></title>
...
<div class="container container-fixed-top-padding">
	<div class="page-header clearfix">
			<h1><bean:message key="HQ.MyDashboard "/></h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div id="profile"  class="box compact">
	<div class="box-title">
		<c:set var="username" value="${miu5odau6q.data.firstName} ${miu5odau6q.data.lastName}"/>
    	<h2 style="color:black !important">${username}&nbsp;</h2>
    	<div class="photo">
			<c:choose>
				<c:when test="${not empty miu5odau6q.data.bigPhoto}">
					<img src="<webratio:BLOB value="${miu5odau6q.data.bigPhoto}" />" alt="${username}"/> 
				</c:when>
				<c:otherwise>
		...
		<div class="information">
			<c:if test="${not empty(miu5odau6q.data.companyName)}">
						<bean:message key="HQ.YourProfile.Work"/> <bean:message key="HQ.YourProfile.At"/> <c:out value="${miu5odau6q.data.companyName}"/>
			</c:if>
			<br/>
			<c:if test="${not empty(miu5odau6q.data.city)}">
				<c:out value="${miu5odau6q.data.city}"/>
			</c:if>
			<c:if test="${not empty(miu5odau6q.data.country)}">
				<c:if test="${not empty(miu5odau6q.data.city)}">, </c:if>
				<c:out value="${miu5odau6q.data.country}"/>
			</c:if>
		</div>
		<div class="important-badge">
 			<c:forEach var="current" varStatus="status" items="${miu5omdu3q.data}">
			...
			<c:if test="${webratio:isTargetAccessible('miu5oln282q', pageContext)}">
<c:if test="${webratio:evaluateCondition('miu5ocexpr4q', null, pageContext)}">
		  		<p> <c:if test="${webratio:evaluateCondition('miu5ocexpr4q', 'index', pageContext)}"><c:if test="${webratio:isTargetAccessible('miu5oln282q', pageContext)}">
	 		<a title="<bean:message key="HQ.YourBadges"/>" id="miu5oln282q" onclick="return ajaxRequest('<webratio:Link link="miu5oln282q" position="index"/>', $H({pressedLink: 'miu5oln282q', sourcePage: 'page20p'}))" class="  read-more linkColorBlue" href="<webratio:Link link="miu5oln282q" position="index"/>">							
...
<div class="credit-box">
    <c:forEach var="current" varStatus="status" items="${miu5omdu2q.data}" >
		<c:set var="index" value="${status.index}"/>
		...
		<p>
			...
			<c:if test="${webratio:isTargetAccessible('miu5oln278q', pageContext)}">
<c:if test="${webratio:evaluateCondition('miu5ocexpr4q', null, pageContext)}">
   			<p> <c:if test="${webratio:evaluateCondition('miu5ocexpr4q', 'index', pageContext)}"><c:if test="${webratio:isTargetAccessible('miu5oln278q', pageContext)}">
	 		<a title="<bean:message key="Credit history"/>" id="miu5oln278q" class="btn  btn-default  read-more" href="<webratio:Link link="miu5oln278q" position="index"/>" >
...
<c:if test="${not(webratio:evaluateCondition('miu5ocexpr6q', null, pageContext))}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
	...
	<div class="panel-body">
	<c:choose>
		<c:when test="${not(empty miu5ohinu14q) and (miu5ohinu14q.dataSize gt 0)}">
		<c:set var="currentUser" value="${not (empty UserCtxParam) and UserCtxParam eq miu5ohinu14q.data[0].OID}"/>
			<p class="short-user-title">
					<c:choose>
						...
						<c:otherwise>
							${miu5ohinu14q.data[0].firstName} ${miu5ohinu14q.data[0].lastName}
							<bean:message key="HQ.is"/>
						</c:otherwise>
					</c:choose>
				<strong>${miu5ohinu14q.scroller.from} <bean:message key="HQ.Of"/> ${miu5ohinu14q.scroller.last}</strong></p>
			<div class="rank-index">
				...
				<c:choose>
				  <c:when test="${miu5ohinu14q.scroller.current eq miu5ohinu14q.scroller.first}">
				    <c:set var="upEnd" value="${-1}"/>
				    <c:choose>
				      <c:when test="${not currentUser}">
				  ...
				  <c:when test="${miu5ohinu14q.scroller.current eq miu5ohinu14q.scroller.last}">
				    <c:choose>
				      <c:when test="${not currentUser}">
				  ...
				  <c:when test="${not currentUser && miu5ohinu14q.scroller.current eq (miu5ohinu14q.scroller.first + 1)}">
				    <c:set var="upEnd" value="${0}"/>
				  	<c:set var="downEnd" value="${2}"/>
				  </c:when>
				  <c:when test="${not currentUser && miu5ohinu14q.scroller.current eq (miu5ohinu14q.scroller.last - 1)}">
				    <c:set var="upEnd" value="${2}"/>
				  	<c:set var="downEnd" value="${0}"/>
				  </c:when>
				</c:choose>
				<c:if test="${not(empty miu5ohinu15q) and (miu5ohinu15q.dataSize gt 0) and upEnd ge 0}">
					<c:forEach var="current" varStatus="level1status" items="${miu5ohinu15q.data}" end="${upEnd}">
						<c:set var="level1" value="${upEnd - level1status.index}" />
						<c:set var="level1current" value="${miu5ohinu15q.data[upEnd - level1status.index]}"/>
						<c:set var="preIndex" value="${miu5ohinu14q.scroller.from - 1 - upEnd + level1status.index}"/>
						<c:if test="${preIndex le 0}">
							<c:set var="preIndex" value="1"/>	
							<c:set var="level1current" value="${miu5ohinu15q.data[0]}"/>
...
<div class="user-rank<c:if test="${index % 2 eq 1}"> alternate</c:if>">
	...
	<div class="photo-place place-n${counter}">
		<c:choose>
	      ...
	      <c:otherwise>
			...
			<a href="<webratio:Link escapeXml="true" link="miu5oln287q" position="level1"/>" title="<bean:message key="community.goToDashboard"/>">
				<c:choose>
	...
	<c:forEach items="${level1current.miu5ohiulvl15qdata}" var="level2current" varStatus="level2status">
		<c:set var="level2" value="${level2status.index}" />
			<div style="display:inline-block;">
				...
				<c:set var="preIndex"/>
				<c:forEach var="level1current" varStatus="level1status" items="${miu5ohinu14q.data}">
					<c:set var="level1" value="${level1status.index}" />
<c:set var="index" value="${level1+1}"/>
<c:if test="${not (empty miu5ohinu14q.scroller)}">
	<c:set var="index" value="${miu5ohinu14q.scroller.from + level1}"/>
</c:if>
<c:if test="${not (empty preIndex)}">
	<c:set var="index" value="${preIndex}"/>
</c:if>
	...
	<div class="photo-place place-n${counter}">
		<c:choose>
	      ...
	      <c:otherwise>
			...
			<a href="<webratio:Link escapeXml="true" link="miu5oln286q" position="level1"/>" title="<bean:message key="community.goToDashboard"/>">
				<c:choose>
					<c:when test="${not empty level1current.smallPhoto}">
						<img src="<webratio:BLOB value="${level1current.smallPhoto}" />"/> 
					</c:when>
		...
		<!-- Badge -->
		<div class="badge-list">
	<c:forEach items="${level1current.miu5ohiulvl14qdata}" var="level2current" varStatus="level2status">
		<c:set var="level2" value="${level2status.index}" />
			<div style="display:inline-block;">
				<div class="tip">
					<c:choose>
				...
				<c:if test="${not(empty miu5ohinu16q) and (miu5ohinu16q.dataSize gt 0) and downEnd ge 0}">
					<c:forEach var="level1current" varStatus="level1status" items="${miu5ohinu16q.data}" end="${downEnd}">
						<c:set var="level1" value="${level1status.index}" />
						<c:set var="preIndex" value="${miu5ohinu14q.scroller.from + 1 + level1}"/>
<c:set var="index" value="${level1+1}"/>
<c:if test="${not (empty preIndex)}">
	<c:set var="index" value="${preIndex}"/>
</c:if>
<div class="user-rank<c:if test="${index % 2 eq 1}"> alternate</c:if>">
	...
	<div class="photo-place place-n${counter}">
		<c:choose>
	      ...
	      <c:otherwise>
			...
			<a href="<webratio:Link escapeXml="true" link="miu5oln288q" position="level1"/>" title="<bean:message key="community.goToDashboard"/>">
				<c:choose>
					<c:when test="${not empty level1current.smallPhoto}">
						<img src="<webratio:BLOB value="${level1current.smallPhoto}" />"/> 
					</c:when>
		...
		<!-- Badge -->
		<div class="badge-list">
	<c:forEach items="${level1current.miu5ohiulvl16qdata}" var="level2current" varStatus="level2status">
		<c:set var="level2" value="${level2status.index}" />
			<div style="display:inline-block;">
				<div class="tip">
					<c:choose>
			...
			<c:if test="${currentUser}">
				<p>
				   <c:if test="${webratio:isTargetAccessible('miu5oln280q', pageContext)}">
	 		<a title="<bean:message key="Go to monthly"/>" id="miu5oln280q" class="btn  btn-default  read-more" href="<webratio:Link link="miu5oln280q" position="index"/>" >
	...
	<c:otherwise>
			<p><c:out value="${miu5odau8q.data.message}"/></p>
	</c:otherwise>
	</c:choose>
	</div>
</div></c:if>
<c:if test="${webratio:evaluateCondition('miu5ocexpr6q', null, pageContext)}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
	...
	<div class="panel-body">
	<c:choose>
		<c:when test="${not(empty miu5ohinu4q) and (miu5ohinu4q.dataSize gt 0)}">
			<div class="rank-index">
			<c:set var="currentUser" value="${not (empty UserCtxParam) and UserCtxParam eq miu5odau6q.data.OID }"/>
		<c:choose>
			      <c:when test="${not currentUser}">
						<c:set var="upEnd" value="${3}"/>
						<c:set var="downEnd" value="${1}"/>
			...
			<c:forEach var="level1current" varStatus="level1status" items="${miu5ohinu4q.data}" begin="${downEnd-1}" end="${upEnd+1}">
								<c:set var="level1" value="${level1status.index}" />
<c:set var="index" value="${level1+1}"/>
<c:if test="${not (empty preIndex)}">
	<c:set var="index" value="${preIndex}"/>
	...
	<div class="photo-place place-n${counter}">
		<c:choose>
	      ...
	      <c:otherwise>
			...
			<a href="<webratio:Link escapeXml="true" link="miu5oln284q" position="level1"/>" title="<bean:message key="community.goToDashboard"/>">
				<c:choose>
					<c:when test="${not empty level1current.smallPhoto}">
						<img src="<webratio:BLOB value="${level1current.smallPhoto}" />"/> 
					</c:when>
		...
		<!-- Badge -->
		<div class="badge-list">
	<c:forEach items="${level1current.miu5ohiulvl4qdata}" var="level2current" varStatus="level2status">
		<c:set var="level2" value="${level2status.index}" />
			<div style="display:inline-block;">
				<div class="tip">
					<c:choose>
		...
		<c:otherwise>
				<c:out value="${miu5odau8q.data.message}" escapeXml="false"/> 
		</c:otherwise>
	</c:choose>
	</div>
</div></c:if>
...
<c:if test="${not(webratio:evaluateCondition('miu5ocexpr5q', null, pageContext))}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
	...
	<div class="panel-body">
	<c:choose>
		<c:when test="${not(empty miu5ohinu17q) and (miu5ohinu17q.dataSize gt 0)}">
		<c:set var="currentUser" value="${not (empty UserCtxParam) and UserCtxParam eq miu5ohinu17q.data[0].OID}"/>
			<p class="short-user-title">
					<c:choose>
						<c:when test="${currentUser}">
							<bean:message key="HQ.YouAre"/>
						...
						<c:otherwise>
							${miu5ohinu17q.data[0].firstName} ${miu5ohinu17q.data[0].lastName}
							<bean:message key="HQ.is"/>
						</c:otherwise>
					</c:choose>
				<strong>${miu5ohinu17q.scroller.from} <bean:message key="HQ.Of"/> ${miu5ohinu17q.scroller.last}</strong></p>
			<div class="rank-index">
			    <c:set var="upEnd" value="${0}"/>
				<c:set var="downEnd" value="${0}"/>
				<c:choose>
				  <c:when test="${miu5ohinu17q.scroller.current eq miu5ohinu17q.scroller.first}">
				    <c:set var="upEnd" value="${-1}"/>
				    <c:set var="downEnd" value="${1}"/>
				  </c:when>
				  <c:when test="${miu5ohinu17q.scroller.current eq miu5ohinu17q.scroller.last}">
				    <c:set var="upEnd" value="${1}"/>
				  	<c:set var="downEnd" value="${-1}"/>
				  </c:when>
				</c:choose>
				<c:if test="${not(empty miu5ohinu18q) and (miu5ohinu18q.dataSize gt 0) and upEnd ge 0}">
					<c:forEach var="current" varStatus="level1status" items="${miu5ohinu18q.data}" end="${upEnd}">
						<c:set var="level1" value="${upEnd - level1status.index}" />
						<c:set var="level1current" value="${miu5ohinu18q.data[upEnd - level1status.index]}"/>
						<c:set var="preIndex" value="${miu5ohinu17q.scroller.from - 1 - upEnd + level1status.index}"/>
						<c:if test="${preIndex le 0}">
							<c:set var="preIndex" value="1"/>	
							<c:set var="level1current" value="${miu5ohinu18q.data[0]}"/>
							<c:set var="level1" value="0"/>
						</c:if>
<c:set var="index" value="${level1+1}"/>
<c:if test="${not (empty preIndex)}">
	...
	<div class="photo-place place-n${counter}">
		<c:choose>
	      ...
	      <c:otherwise>
			...
			<a href="<webratio:Link escapeXml="true" link="miu5oln290q" position="level1"/>" title="<bean:message key="community.goToDashboard"/>">
				<c:choose>
					<c:when test="${not empty level1current.smallPhoto}">
						<img src="<webratio:BLOB value="${level1current.smallPhoto}" />"/> 
					</c:when>
		...
		<!-- Badge -->
		<div class="badge-list">
	<c:forEach items="${level1current.miu5ohiulvl18qdata}" var="level2current" varStatus="level2status">
		<c:set var="level2" value="${level2status.index}" />
			<div style="display:inline-block;">
				<div class="tip">
					<c:choose>
				...
				<c:set var="preIndex"/>
				<c:forEach var="level1current" varStatus="level1status" items="${miu5ohinu17q.data}">
					<c:set var="level1" value="${level1status.index}" />
<c:set var="index" value="${level1+1}"/>
<c:if test="${not (empty miu5ohinu17q.scroller)}">
	<c:set var="index" value="${miu5ohinu17q.scroller.from + level1}"/>
</c:if>
<c:if test="${not (empty preIndex)}">
	<c:set var="index" value="${preIndex}"/>
</c:if>
	...
	<div class="photo-place place-n${counter}">
		<c:choose>
	      ...
	      <c:otherwise>
			...
			<a href="<webratio:Link escapeXml="true" link="miu5oln289q" position="level1"/>" title="<bean:message key="community.goToDashboard"/>">
				<c:choose>
					<c:when test="${not empty level1current.smallPhoto}">
						<img src="<webratio:BLOB value="${level1current.smallPhoto}" />"/> 
					</c:when>
		...
		<!-- Badge -->
		<div class="badge-list">
	<c:forEach items="${level1current.miu5ohiulvl17qdata}" var="level2current" varStatus="level2status">
		<c:set var="level2" value="${level2status.index}" />
			<div style="display:inline-block;">
				<div class="tip">
					<c:choose>
				...
				<c:if test="${not(empty miu5ohinu19q) and (miu5ohinu19q.dataSize gt 0) and downEnd ge 0}">
					<c:forEach var="level1current" varStatus="level1status" items="${miu5ohinu19q.data}" end="${downEnd}">
						<c:set var="level1" value="${level1status.index}" />
						<c:set var="preIndex" value="${miu5ohinu17q.scroller.from + 1 + level1}"/>
<c:set var="index" value="${level1+1}"/>
<c:if test="${not (empty preIndex)}">
	<c:set var="index" value="${preIndex}"/>
</c:if>
	...
	<div class="photo-place place-n${counter}">
		<c:choose>
	      ...
	      <c:otherwise>
			...
			<a href="<webratio:Link escapeXml="true" link="miu5oln291q" position="level1"/>" title="<bean:message key="community.goToDashboard"/>">
				<c:choose>
					<c:when test="${not empty level1current.smallPhoto}">
						<img src="<webratio:BLOB value="${level1current.smallPhoto}" />"/> 
					</c:when>
		...
		<!-- Badge -->
		<div class="badge-list">
	<c:forEach items="${level1current.miu5ohiulvl19qdata}" var="level2current" varStatus="level2status">
		<c:set var="level2" value="${level2status.index}" />
			<div style="display:inline-block;">
				<div class="tip">
					<c:choose>
			...
			<c:if test="${currentUser}">
				<p>
				   <c:if test="${webratio:isTargetAccessible('miu5oln281q', pageContext)}">
	 		<a title="<bean:message key="Go to participation"/>" id="miu5oln281q" class="btn  btn-default  read-more" href="<webratio:Link link="miu5oln281q" position="index"/>" >
...
<c:if test="${webratio:evaluateCondition('miu5ocexpr5q', null, pageContext)}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
	...
	<div class="panel-body">
	<c:choose>
		<c:when test="${not(empty miu5ohinu13q) and (miu5ohinu13q.dataSize gt 0)}">
			<div class="rank-index">
			<c:set var="currentUser" value="${not (empty UserCtxParam) and UserCtxParam eq miu5odau6q.data.OID }"/>
		<c:choose>
			      <c:when test="${not currentUser}">
			...
			<c:forEach var="level1current" varStatus="level1status" items="${miu5ohinu13q.data}" begin="${downEnd-1}" end="${upEnd+1}">
								<c:set var="level1" value="${level1status.index}" />
<c:set var="index" value="${level1+1}"/>
<c:if test="${not (empty preIndex)}">
	<c:set var="index" value="${preIndex}"/>
...
<div class="user-rank<c:if test="${index % 2 eq 1}"> alternate</c:if>">
	...
	<div class="photo-place place-n${counter}">
		<c:choose>
	      ...
	      <c:otherwise>
			...
			<a href="<webratio:Link escapeXml="true" link="miu5oln285q" position="level1"/>" title="<bean:message key="community.goToDashboard"/>">
				<c:choose>
					<c:when test="${not empty level1current.smallPhoto}">
						<img src="<webratio:BLOB value="${level1current.smallPhoto}" />"/> 
					</c:when>
		...
		<!-- Badge -->
		<div class="badge-list">
	<c:forEach items="${level1current.miu5ohiulvl13qdata}" var="level2current" varStatus="level2status">
		<c:set var="level2" value="${level2status.index}" />
			<div style="display:inline-block;">
				<div class="tip">
					<c:choose>
	...
	<script type="application/json" class="wr-linkInfos">
		<webratio:LinkInfos page="page20p"/>
	</script>
	<script type="application/json" class="wr-linkData">
		<webratio:LinkData page="page20p"/>
	</script>
