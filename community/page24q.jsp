...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page24q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page24qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page24q"/>
				<input type="hidden" name="ln81q" value="<webratio:Link link="ln81q"/>">
				<input type="hidden" name="ln83q" value="<webratio:Link link="ln83q"/>">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">New Thematic Area</li>
	</ol>
	<div class="page-header clearfix">
			<h1>New Thematic Area</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu14q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu14q"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld36q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld36q" class="control-label col-md-2">Area Name</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld36q" styleClass="wr-submitButtons:ln81q,ln83q  form-control " property="fld36q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld36q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="msfld3q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="msfld3q" class="control-label col-md-2">Applications Associated</label>
					<div class="controls col-md-10">
					<c:forEach var="current" varStatus="status" items="${page24qFormBean.map.msfld3qList}">
				 <label class="checkbox-inline" for="msf_msfld3q_${status.index}">
				  <html:multibox styleClass="wr-submitButtons:ln81q,ln83q " property="msfld3q" value="${current}" disabled="false" styleId="msf_msfld3q_${status.index}"/>
					   <c:out value="${page24qFormBean.map.msfld3qLabelList[status.index]}"/>
				 </label>
			</c:forEach>
					<c:set var="btFieldError"><html:errors property="msfld3q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln81q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln81q', pageContext)}">
			<button title="Save" onclick="$('#page24qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln81q" name="button:ln81q" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln83q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln83q', pageContext)}">
			<button title="Back" onclick="$('#page24qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln83q" name="button:ln83q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
