...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page19q_miu77q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page19q_miu77qFormBean" enctype="multipart/form-data">
...
<div class="EntryUnit ">
	<html:hidden property="enu12qKey" styleId="enu12qKey"></html:hidden>
		<html:hidden property="fld35q" styleId="fld35q"/>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu12q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				...
				<c:set var="btFieldError"><html:errors property="fld33q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld33q" class="control-label col-md-2">Key</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld33q" styleClass="wr-submitButtons:ln232q,ln231q  form-control " property="fld33q" readonly="true"/>
					<c:set var="btFieldError"><html:errors property="fld33q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld34q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld34q" class="control-label col-md-2">Message</label>
					<div class="controls col-md-10">
					<html:textarea styleId="fld34q" styleClass="form-control " rows="8" property="fld34q" readonly="false"/>		
			<script type="text/javascript">
				...
				CKEDITOR.replace('fld34q', {
					...
					<c:set var="btFieldError"><html:errors property="fld34q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln232q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln232q', pageContext)}">
			<button title="Save" onclick="$('#page19q_miu77qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln232q" name="button:ln232q" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln231q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln231q', pageContext)}">
			<button title="Back" onclick="$('#page19q_miu77qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln231q" name="button:ln231q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
