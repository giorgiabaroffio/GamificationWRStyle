...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page10q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page10qFormBean" enctype="multipart/form-data">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page7q.link"/>">
   Administrator Users Area
		...
		<li class="active">Modify Adminstrator User</li>
	</ol>
	<div class="page-header clearfix">
			<h1>Modify Adminstrator User</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div class="EntryUnit ">
	...
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu6q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				...
				<c:set var="btFieldError"><html:errors property="fld15q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld15q" class="control-label col-md-2">Email</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld15q" styleClass="wr-submitButtons:ln114q,ln110q  form-control " property="fld15q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld15q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld16q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld16q" class="control-label col-md-2">Password</label>
					<div class="controls col-md-10">
					<input type="password" autocomplete="off" id="fld16q" class="wr-submitButtons:ln114q,ln110q form-control " name="fld16q"/>
					<c:set var="btFieldError"><html:errors property="fld16q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln114q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln114q', pageContext)}">
			<button title="Update" onclick="$('#page10qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln114q" name="button:ln114q" type="submit" value="Update">
			Update</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln110q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln110q', pageContext)}">
			<button title="Back" onclick="$('#page10qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln110q" name="button:ln110q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
