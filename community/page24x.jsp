...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page24x.do${wrAjax ? '#!ajax=true' : ''}" styleId="page24xFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page24x"/>
				<input type="hidden" name="ln178x" value="<webratio:Link link="ln178x"/>">
				<input type="hidden" name="ln194x" value="<webratio:Link link="ln194x"/>">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">Modify Badge</li>
	</ol>
...
<div class="EntryUnit ">
	<html:hidden property="enu13xKey" styleId="enu13xKey"></html:hidden>
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu13x"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu13x"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld42x_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld42x_locale" class="control-label col-md-2">Level</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld42x_locale" styleClass="wr-submitButtons:ln194x,ln178x  form-control " property="fld42x_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld42x_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld46x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld46x" class="control-label col-md-2">Title</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld46x" styleClass="wr-submitButtons:ln194x,ln178x  form-control " property="fld46x" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld46x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld47x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld47x" class="control-label col-md-2">Description</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld47x" styleClass="wr-submitButtons:ln194x,ln178x  form-control " property="fld47x" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld47x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld45x_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld45x_locale" class="control-label col-md-2">Needed Score</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld45x_locale" styleClass="wr-submitButtons:ln194x,ln178x  form-control " property="fld45x_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld45x_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld39x"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld39x" class="control-label col-md-2">Image (png)</label>
					<div class="controls col-md-10">
					<html:hidden property="fld39x_preload" styleId="fld39x_preload"/>
			<html:file styleId="fld39x" styleClass="wr-submitButtons:ln194x,ln178x " property="fld39x" accept="image/*" disabled="false" />
		<c:if test="${not(empty page24xFormBean.map.fld39x_preload)}">
			<label class="input-group-addon blob-clear" for="fld39x_clear"><html:checkbox styleClass="wr-submitButtons:ln194x,ln178x " styleId="fld39x_clear" property="fld39x_clear" disabled="false"/> Clear </label><div class="input-group-btn"><a target="blank" class="btn btn-default" href="<webratio:BLOB value="${page24xFormBean.map.fld39x_preload}"/>">Preview</a></div>
		</c:if>
					<c:set var="btFieldError"><html:errors property="fld39x"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld24q_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld24q_locale" class="control-label col-md-2">Active</label>
					<div class="controls col-md-10">
					<label class="radio-inline" for="bool_fld24q_0">
  					<html:radio styleClass="wr-submitButtons:ln194x,ln178x " styleId="bool_fld24q_0" property="fld24q_locale" value="true" disabled="false"/>
					<bean:message key="boolean.yes"/>
				</label>
				<label class="radio-inline" for="bool_fld24q_1">
  					<html:radio styleClass="wr-submitButtons:ln194x,ln178x " styleId="bool_fld24q_1" property="fld24q_locale" value="false" disabled="false"/>
					<bean:message key="boolean.no"/>
				</label>
		<c:if test="false">
			<html:hidden property="fld24q_locale"/>
		</c:if>
					<c:set var="btFieldError"><html:errors property="fld24q_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln194x', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln194x', pageContext)}">
			<button title="Update" onclick="$('#page24xFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln194x" name="button:ln194x" type="submit" value="Update">
			Update</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln178x', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln178x', pageContext)}">
			<button title="Back" onclick="$('#page24xFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln178x" name="button:ln178x" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
