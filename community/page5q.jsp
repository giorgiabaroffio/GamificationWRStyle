...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page5q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page5qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page5q"/>
				<input type="hidden" name="ln25q" value="<webratio:Link link="ln25q"/>">
				<input type="hidden" name="ln28q" value="<webratio:Link link="ln28q"/>">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">New Badge Area</li>
	</ol>
	<div class="page-header clearfix">
			<h1>New Badge Area</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu4q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu4q"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="sfld8q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="sfld8q" class="control-label col-md-2">Thematic Area</label>
					<div class="controls col-md-10">
					<html:select styleId="sfld8q" styleClass="wr-submitButtons:ln25q,ln28q form-control " property="sfld8q" disabled="false">
				  		<html:option value=""><bean:message key="selectionfield.noselection"/></html:option>
				    <html:options property="sfld8qList" labelProperty="sfld8qLabelList"/>
				</html:select>
					<c:set var="btFieldError"><html:errors property="sfld8q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld8q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld8q" class="control-label col-md-2">Sort Number</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld8q" styleClass="wr-submitButtons:ln25q,ln28q  form-control " property="fld8q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld8q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld7q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld7q" class="control-label col-md-2">Name</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld7q" styleClass="wr-submitButtons:ln25q,ln28q  form-control " property="fld7q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld7q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln25q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln25q', pageContext)}">
			<button title="Save" onclick="$('#page5qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln25q" name="button:ln25q" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln28q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln28q', pageContext)}">
			<button title="Back" onclick="$('#page5qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln28q" name="button:ln28q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
