...
<webratio:Page page="page3q"/>
<html lang="${LanguageISOCtxParam}">
	<head>
		<meta http-equiv="content-type" content="text/html; charset=utf-8">
	    <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<base href="<%=wrBaseURI%>"/> 
		<title>New Gamified Application</title>
...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page3q.do${wrAjax ? '#!ajax=true' : ''}" styleId="page3qFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page3q"/>
				<input type="hidden" name="ln20q" value="<webratio:Link link="ln20q"/>">
				<input type="hidden" name="ln21q" value="<webratio:Link link="ln21q"/>">
...
<div class="container container-fixed-top-padding">
 ...
 <a class="small" href="<webratio:Link link="page1q.link"/>">
   Gamification Area
		...
		<li class="active">New Gamified Application</li>
	</ol>
	<div class="page-header clearfix">
			<h1>New Gamified Application</h1>
	</div>
	<div class="row">
		<div class="col-md-12">
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu2q"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu2q"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld3q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld3q" class="control-label col-md-2">Name</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld3q" styleClass="wr-submitButtons:ln20q,ln21q  form-control " property="fld3q" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld3q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="msfld4q"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="msfld4q" class="control-label col-md-2">Thematic Area Associated</label>
					<div class="controls col-md-10">
					<c:forEach var="current" varStatus="status" items="${page3qFormBean.map.msfld4qList}">
				 <label class="checkbox-inline" for="msf_msfld4q_${status.index}">
				  <html:multibox styleClass="wr-submitButtons:ln20q,ln21q " property="msfld4q" value="${current}" disabled="false" styleId="msf_msfld4q_${status.index}"/>
					   <c:out value="${page3qFormBean.map.msfld4qLabelList[status.index]}"/>
				 </label>
			</c:forEach>
					<c:set var="btFieldError"><html:errors property="msfld4q"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
			<c:if test="${webratio:isTargetAccessible('ln20q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln20q', pageContext)}">
			<button title="Save" onclick="$('#page3qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln20q" name="button:ln20q" type="submit" value="Save">
			Save</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln21q', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln21q', pageContext)}">
			<button title="Back" onclick="$('#page3qFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln21q" name="button:ln21q" type="submit" value="Back">
			Back</button>
	</c:if>
			</c:if>
		</div>
